...
max_r2 <- "0.952"
mean_r2 <- "0.344"
r_version <- substr(R.version[[13]], 11,15)
```

# Mackenzie Mountains Climate Data Archiving Project
Prepared 6 January 2025 by Steven Mamet^1,2^  
...
ft1 <- qflextable(tab1) %>% 
  set_caption(caption = as_paragraph(sprintf(
    "Table 1. Record completion details prior to gap filling for the daily temperature records measured along the Canol Heritage Trail from %s to %s", field_year - 1, field_year)),
    style = "Table Caption") %>%
  hline_bottom() %>%
  # hline(i = c(1)) %>%
   footnote(i = 1, j = 2,
...
ft2 <- qflextable(tab2) %>% 
  set_caption(caption = as_paragraph(sprintf(
    "Table 2. Record completion details for the daily liquid precipitation records measured along the Canol Heritage Trail from %s to %s", field_year - 1, field_year)),
    style = "Table Caption") %>%
  hline_bottom()
```
```{r echo = FALSE}
